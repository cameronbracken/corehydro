...
//
// The nineteen groups (correlation, gof, statistics, spectra, histogram, interpolation,
// regression, sampling, probability, link, trend, linalg, special, functions, network,
// hypothesis, paired_data, ml, timeseries) each live in their own header under
// numerics/support/toolbox/, holding
...
#include "corehydro/numerics/support/toolbox/ml.hpp"
#include "corehydro/numerics/support/toolbox/timeseries.hpp"
...
inline ToolboxResult run_toolbox(const std::string& group, const std::string& method,
    ...
    if (group == "ml") return detail::run_ml(method, data, options);
    if (group == "timeseries") return detail::run_timeseries(method, data, options);
