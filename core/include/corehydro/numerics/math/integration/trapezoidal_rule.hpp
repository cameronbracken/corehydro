// ported from: Numerics/Mathematics/Integration/TrapezoidalRule.cs @ 2a0357a
//
// Trapezoidal rule integration by repeated refinement: `Next()` doubles the number of
// trapezoidal panels each call (identical helper to SimpsonsRule's, duplicated rather than
// shared since the two C# classes duplicate it too), with no Simpson/Richardson correction
// applied to the successive estimates. Iteration continues until `EvaluateConvergence` (both
// absolute and relative tolerance) is satisfied, `MaxFunctionEvaluations` is hit, or
...
class TrapezoidalRule : public Integrator {
   public:
    /// Constructs a new trapezoidal rule class. `function` is the function to integrate, `min`
    /// and `max` the bounds the integral is computed under.
    TrapezoidalRule(std::function<double(double)> function, double min, double max) {
    ...
    void integrate() override {
        ...
        try {
            ...
            for (int i = 0; i < max_iterations; i++) {
                ...
                // Check convergence
                if (i > 2) {
                    if (evaluate_convergence(s1, s0) || (s0 == 0.0 && s1 == 0.0)) {
                        result_ = s0;
