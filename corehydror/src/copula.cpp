// cpp11 glue for the one bivariate-copula fixture case the shared distribution runner cannot
// serve. Every user-facing copula verb -- and every fitted copula -- now goes through
// ch_copula_run_ in dist_spec.cpp, which drives the same dist_runner.hpp entry point the C++
// fixture runner, the Python glue and the dotnet oracle emitter drive. What is left here is
// the non-finite theta/df validity cases: the runner's JSON grammar has no NaN/Infinity
// literal, so those constructs are built directly through the factory instead. Every copula
...
// marginX, marginY)` ctor path): "" means no marginal for that side.
//
// Only `parameters_valid` is reached today (the non-finite theta/df cases described in the file
// header); the other arms are kept so this stays a faithful fallback for any construct the
// shared grammar cannot encode, exactly as the fixture runner's own bespoke dispatchers do.
[[cpp11::register]]
