...
layer is not yet ported (planned for a future release, see the `MachineLearning` row
below) -- it is what blocks `HypothesisTests.UnimodalityTest`, see the `Data.Statistics`
row below.
...
| `Data.Statistics` | Partial | Public | MGBT, Box-Cox, Yeo-Johnson, plotting positions, descriptive and running statistics, correlation (paired and, since this phase, the Pearson/Spearman matrix form), autocorrelation, goodness of fit, histogram binning, and joint probability all public; `hypothesis_test()` exposes twelve of the thirteen `HypothesisTests` statics (one/two-sample t, F, Jarque-Bera, Wald-Wolfowitz, Ljung-Box, Mann-Whitney, Mann-Kendall, linear trend) -- `UnimodalityTest`, the thirteenth, is deferred (it needs the unported `MachineLearning.GaussianMixtureModel`); single Grubbs-Beck, KS tests, and Jenks breaks not ported |
...
| `Models` (data frame, censored data, trends, all model families) | Ported | Public | flood frequency, Bulletin 17C, mixtures, competing risks, point process, AR/MA/ARIMA/ARIMAX, spatial GEV, rating curves, bivariate; `analysis_data_hypothesis_test()`/`analysis_data_statistics()` expose twelve of the DataFrame's fourteen hypothesis-test/summary-statistics facade members (`UnimodalityTest`/`SummaryHypothesisTest` need the still-unported `GaussianMixtureModel`) |
