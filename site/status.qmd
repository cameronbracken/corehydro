...
| `Models` (data frame, censored data, trends, all model families) | Ported | Public | flood frequency, Bulletin 17C, mixtures, competing risks, point process, AR/MA/ARIMA/ARIMAX, spatial GEV, rating curves, bivariate; `analysis_data_hypothesis_test()`/`analysis_data_statistics()` expose eleven of the DataFrame's twelve hypothesis-test/summary-statistics facade members (`UnimodalityTest`/`SummaryHypothesisTest` need the still-unported `GaussianMixtureModel`) |
