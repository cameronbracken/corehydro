...
never-fused arithmetic on the last bit of either. Measured against the real C# library when the port
landed (`fixtures/callback/math.json`'s `quadrature_miser_gsl` case): Miser's own `Result` misses
the C# value by 1 ULP under this package's shipped build. Measured directly R against Python at the
