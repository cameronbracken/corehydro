...
`Numerics.Data.Statistics.GoodnessOfFit`. That class is ported, and `gof_test()` reaches
its KS, Anderson-Darling and chi-squared statistics (example 12 works through them), but
this page ranks by log-likelihood and AIC instead, because both penalize the extra
parameter GEV spends and KS does not. The log-likelihoods are the exact values the script
also prints, and the conclusion is the same either way: Weibull edges out GEV, and
LogNormal trails.
...
goodness-of-fit panel shows dAIC instead of the KS statistic, for the reason above.
